...
<style>
	 body {
	 	background-image : url("http://70.12.113.161:8000/mvc/images/aa.png");
...
<body>
	...
	%>
		<tr>
			<td><%=vo.getId() %></td>
			<td>
				<a href="http://70.12.113.161:8000/mvc/news?action=read&newsid=<%=vo.getId()%>"><%=vo.getTitle() %></a>
			</td>
			<td>
				<a href="http://70.12.113.161:8000/mvc/news?action=listwriter&writer=<%=vo.getWriter()%>"><%=vo.getWriter() %></a>
			</td>
	...
	<form method="get" action="http://70.12.113.161:8000/mvc/news">
	...
	<% 
		...
		if(isSearch != null) {
	%>
		<button id="newsBtn">뉴스 홈</button>
	<%
		}
	%>
	<button id="writeBtn" value="false">뉴스 작성</button>
	<div id="writeForm" style="display:none">
		<form method="post" action="http://70.12.113.161:8000/mvc/news">
			<input type="hidden" name="action" value="insert" id="hidden">
			<input type="hidden" name="id" id="id">
			<input type="text" name="writer" id="writer" placeholder="작성자명을 입력해주세요"><br>
			<input type="text" name="title" id="title" placeholder="제목을 입력해주세요"><br>
			<textarea name="content" id="content" placeholder="내용을 입력해주세요" rows="10" cols="50"></textarea><br>
			
			<div id="btnSet1">
				<button type="submit" id="btn1">저장</button>
				<button type="reset" id="btn2">재작성</button>
				<button type="button" id="btn3" onclick="$('#writeForm').css('display', 'none')">취소</button>
			</div>
	...
	%>
		...
		<script>
			...
			$("#btn6").click(function(){
				location.href = "http://70.12.113.161:8000/mvc/news?action=delete&newsid=<%=vo.getId()%>";
			});
		</script>
	<%
		}
